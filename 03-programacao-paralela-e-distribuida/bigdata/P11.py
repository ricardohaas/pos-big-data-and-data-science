# https://github.com/bragisig/python-sleeping-barber/blob/master/sleeping_barber.py
...
class BarberShop:
    ...
    def __init__(self, barber, numberOfSeats):
        ...
        print(
        'BarberShop initilized with {0} seats'.format(numberOfSeats))
        print(
        'Customer min interval {0}'.format(customerIntervalMin))
        print(
        'Customer max interval {0}'.format(customerIntervalMax))
        print(
        'Haircut min duration {0}'.format(haircutDurationMin))
        print(
        'Haircut max duration {0}'.format(customerIntervalMax))
        print(
        '---------------------------------------')
    ...
    def enterBarberShop(self, customer):
        ...
        if len(self.waitingCustomers) == self.numberOfSeats:
            ...
            print(
            '{0} sat down in the waiting room'.format(customer.name))
